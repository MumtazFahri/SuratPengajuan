...
<body>
    ...
    <div class="dashboard-container">
        ...
        <div class="text-center mb-4 fade-in">
            ...
            <p class="text-white-50" style="color: #a16207 !important;">Kelola WhatsApp Broadcast Server dari Browser</p>
        </div>
...
<div class="card fade-in">
    ...
    <div class="card-body">
        ...
        <div class="row mb-4">
            ...
            <!-- ✅ BUTTON LOGOUT BARU -->
            <div class="col-md-4 mb-3">
                <button class="btn btn-warning btn-custom w-100" id="btn-logout-wa" onclick="logoutWhatsApp()" <?= !$server_status['ready'] ? 'disabled' : '' ?>>
                    <i class="fas fa-sign-out-alt"></i> <span id="logout-text">Logout WhatsApp</span>
                </button>
            </div>
        </div>
        ...
        <div class="instruction-box">
            ...
            <ol class="mb-0 mt-2">
                ...
                <li><strong>Klik "Logout WhatsApp"</strong> untuk disconnect tanpa stop server</li>
                <li><strong>Klik "Stop Server"</strong> untuk menghentikan server (otomatis logout)</li>
            </ol>
        </div>
    </div>
</div>
                            ...
                            <?php if (!empty($recipients)): ?>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <button class="btn btn-sm btn-warning" onclick="editRecipient(<?= $r->id ?>)" title="Edit" style="background-color: #f59e0b; color: white; border: none;">
                                        ...
                                        <button class="btn btn-sm btn-danger" onclick="deleteRecipient(<?= $r->id ?>)" title="Hapus" style="border: none; color: white;">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" style="background-color: #9ca3af; border: none; color: white;">Batal</button>
                        <button type="button" class="btn btn-primary" onclick="saveRecipient()" style="background-color: #fb923c; border: none; color: white;">
    ...
    <script>
        ...
        socket.on('status', (data) => {
            ...
            // ✅ TAMBAHKAN INI
            else if (data.status === 'reinitializing') {
                updateWAStatus('reinitializing', 'Reinitializing...');
                showWAConnectionProgress();
                updateWAConnectionProgress(30, 'Reinitializing for new QR...', 2);
            }
        });
...
function updateWAStatus(status, message) {
    ...
    switch(status) {
        ...
        case 'disconnected':
            ...
            break;
        // ✅ TAMBAHKAN STATUS BARU INI
        case 'reinitializing':
            badge.className = 'status-badge';
            badge.style.backgroundColor = '#dbeafe';
            badge.style.color = '#1e40af';
            badge.style.borderColor = '#3b82f6';
            badge.innerHTML = '<i class="fas fa-sync fa-spin"></i> Reinitializing...';
            break;
        default:
            badge.className = 'status-badge';
        ...
        // Fungsi Stop Server
        function stopServer() {
            if (!confirm('Hentikan server WhatsApp? Koneksi akan terputus.')) {
                return;
            }
            
            // Show loading bar
            showLoading('Stopping Server', 'Server WhatsApp sedang dihentikan...', [
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.online && response.is_running) {
                ...
                if (response.ready) {
                    ...
                    // ✅ Enable logout button kalau WhatsApp connected
                    document.getElementById('btn-logout-wa').disabled = false;
                } else {
                    updateWAStatus(response.whatsapp_status, 'Checking...');
                    addLog('⚠️ WhatsApp not ready yet', 'warning');
                    
                    // ✅ Disable logout button kalau belum connected
                    document.getElementById('btn-logout-wa').disabled = true;
                ...
                document.getElementById('btn-logout-wa').disabled = true; // ✅ Disable logout kalau server offline
            }
        },
        error: function() {
            updateServerStatus(false);
            ...
            document.getElementById('btn-start-server').disabled = false;
            document.getElementById('btn-stop-server').disabled = true;
            document.getElementById('btn-logout-wa').disabled = true; // ✅ Disable logout kalau error
        }
    });
}
       ...
       // ✅ FUNGSI LOGOUT WHATSAPP (UPDATE)
function logoutWhatsApp() {
    if (!confirm('Logout dari WhatsApp? Anda perlu scan QR lagi untuk menghubungkan ulang.')) {
        return;
    }
    
    // Show loading bar
    showLoading('Logging out WhatsApp', 'WhatsApp sedang logout. QR baru akan muncul sebentar lagi...', [
        { id: 1, text: 'Disconnecting from WhatsApp...' },
        { id: 2, text: 'Clearing session...' },
        { id: 3, text: 'Reinitializing for new QR...' }
    ]);
    ...
    const btn = document.getElementById('btn-logout-wa');
    const btnText = document.getElementById('logout-text');
    const originalText = btnText.textContent;
    
    btn.disabled = true;
    btnText.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span>Logging out...';
    
    addLog('🚪 Logging out WhatsApp...', 'warning');
    
    $.ajax({
        url: '<?= base_url("whatsapp/logout_whatsapp") ?>',
        method: 'POST',
        dataType: 'json',
        timeout: 10000,
        success: function(response) {
            ...
            if (response.success) {
                ...
                setTimeout(() => {
                    ...
                    // Update status
                    updateWAStatus('reinitializing', 'Preparing new QR...');
                    
                    // ✅ TAMPILKAN PROGRESS BAR WA CONNECTION
                    showWAConnectionProgress();
                    updateWAConnectionProgress(30, 'Reinitializing for new QR...', 2);
                    
                    showAlert('success', response.message);
                    
                    // ✅ Auto-check status lebih sering untuk deteksi QR baru
                    let checkCount = 0;
        ...
        error: function(xhr, status, error) {
            ...
            setTimeout(() => {
                ...
                btnText.textContent = originalText;
                addLog('❌ Error: ' + error, 'error');
                showAlert('danger', 'Gagal logout: ' + error);
            }, 1000);
        }
    });
}
